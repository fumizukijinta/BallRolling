...
- 各実装ステップ完了時に、発生した問題と解決を `C:\Unity\templates\lessons-learned.md` の**既存テンプレ形式**（4セクション構成）で「## Step N」セクションへ追記する
...
| エージェント | 役割 | 使用場面 |
|---|---|---|
| `game-designer` | 要件整理・設計書更新 | 新機能・仕様変更・設計レビュー |
| `game-coder` | 実装 | 実装ステップ・リファクタリング |
| `game-tester` | テスト作成・実行・失敗分析 | テスト追加・実行、失敗の切り分け |
| `bug-investigator` | 再現・診断・原因特定（修正しない） | バグ報告の最初の調査 |
| `bug-fixer` | 修正の実装 | 調査結果を受けた修正 |
...
- **実機プレイ確認は省略しない**: スモークテスト合格＝実プレイ合格ではない（呼び出しコンテキストの差で偽陽性が出る実例2件）。ユーザーの実プレイ確認を最終ゲートとする
